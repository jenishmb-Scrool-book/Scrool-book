...
Картинки для лент. Три шага, каждый запускается отдельно:
    ...
    python tools/fetch-pics.py --walls     обои домашнего экрана, отдельно и крупно
...
# Обои домашнего экрана — отдельный набор, а не двадцать снимков, взятых из
# вертикальной ленты. Причина одна, и она про размер.
#
# В ленте вертикальный кадр показывается полосой шириной 375 точек, и 432x768
# при качестве 46 там не отличить от оригинала. Обои растягиваются на весь
# экран телефона — это 1080 точек в ширину, — и тот же файл приходится
# увеличивать в два с половиной раза вместе со всеми огрехами сжатия. Пиксели
# в шесть раз крупнее, а качества в них столько же, сколько было в миниатюре.
#
# Соотношение 9:19.5, а не 9:16 как в ленте: у сегодняшних телефонов экран
# вытянут именно так. Кадр 9:16, растянутый `cover` на 9:20, теряет четверть
# ширины — обои показывали бы не то, что человек выбрал в решётке.
#
# Размер просим у сервера сразу нужный: он режет и уменьшает из оригинала в
# одно действие. Промежуточный файл добавил бы второе уменьшение, а с ним мыло;
# проверено на трёх снимках — резкость падает, вес тот же.
#
# Номера заморожены здесь и повторно из раскладки не выводятся: раскладка
# зависит от того, какие снимки в тот день скачались, а эти двадцать выбраны
# просмотром всех 195 вертикальных. Порядок — тот же, что в `src/wallpaper.js`:
# файл `wall/007.webp` и седьмая плитка решётки — это один и тот же снимок.
WALL_IDS = [465, 1038, 810, 269, 905, 27, 505, 412, 1041, 141,
            876, 507, 994, 653, 857, 509, 773, 416, 902, 704]
WALL = ('wall', 1080, 2340, 74)       # папка, ширина, высота, качество

# Миниатюра к каждым обоям. Нужна потому, что решётка выбора показывает все
# двадцать штук разом: двадцать картинок по 1080x2340 — это четверть гигабайта
# распакованных пикселей в памяти телефона ради плиток шириной 64 точки.
#
# Соотношение 9:16, а не 9:19.5 как у самих обоев: плитка 9:19.5 при пяти
# столбцах вытягивается так, что четыре ряда не помещаются на экран первого
# запуска. Плитка показывает середину кадра — то же, что окажется в середине
# экрана. Берётся из того же скачанного файла, отдельной загрузки нет.
THUMB = ('wall/small', 216, 384, 56)
...
def wall_one(job):
    """Одни обои: скачать в нужном размере и сжать. Возвращает вес файла."""
    k, pid = job
    name, w, h, q = WALL
    ...
    try:
        ...
        if im.size != (w, h):         # сервер отдал не то — дорезаем сами
            im = cut(im, w, h)
        im.save(dst, 'WEBP', quality=q, method=6)
        tname, tw, th, tq = THUMB
        tdst = os.path.join(OUT, tname, '%03d.webp' % k)
        cut(im, tw, th).save(tdst, 'WEBP', quality=tq, method=6)
        return os.path.getsize(dst) + os.path.getsize(tdst)
    except Exception as e:
        print('  %03d (id %d): %s' % (k, pid, e))
        return 0


def do_walls():
    """Обои. Папка НЕ чистится перед скачиванием намеренно: имена постоянные,
    файл перезаписывается, а упавшая сеть иначе стёрла бы то, что уже в сборке."""
    name, w, h, q = WALL
    os.makedirs(os.path.join(OUT, name), exist_ok=True)
    os.makedirs(os.path.join(OUT, THUMB[0]), exist_ok=True)
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        sizes = list(pool.map(wall_one, enumerate(WALL_IDS, 1)))
    ok = [s for s in sizes if s]
    print('  oboi %d/%d sht, %dx%d q%d, %.2f MB, %.0f kB srednee'
          % (len(ok), len(sizes), w, h, q,
             sum(ok) / 1048576.0, sum(ok) / max(1, len(ok)) / 1024.0))
