...
// Все операции идут через очередь: initNative может быть вызван повторно до
// того, как первый вызов дождался своего PluginListenerHandle (перемонтировали
// эффект, случился HMR). Очередь гарантирует, что снятие старой подписки
// всегда завершится раньше, чем начнётся установка новой.
//
// Эти две функции однажды уже пропали отсюда — патч, добавлявший работу со
// статус-баром, перезаписал их вместо того, чтобы встать рядом. Ссылки на них
// остались, и `initNative` на телефоне падал первой же строкой: ReferenceError,
// подписки на «назад» нет, кнопка молчит. В браузере этого не видно — там
// initNative выходит раньше, на проверке платформы. Поэтому у файла теперь
// есть свой тест, который поднимает его как на телефоне.
...
/**
 * Что вышло из попытки включить напоминание.
 *
 * Не «получилось / нет», потому что «не получилось» бывает по трём разным
 * причинам, и человеку от них разное. Отказ ОС чинится в настройках телефона;
 * браузер не чинится вовсе — там будильника нет и быть не может; сорванная
 * запись — это «попробуй ещё раз». Один ответ на три случая уже соврал:
 * в браузере переключатель винил в отказе Android, которого там нет.
 */
export const NOTIFY = {on: 'on', denied: 'denied', nowhere: 'nowhere', failed: 'failed'};

/** Ставит будильник на ближайшие 12:00. Разрешение к этому моменту уже есть. */
async function schedule(plugin, payload) {
...
/**
 * Переставить уже включённое напоминание, ничего не спрашивая.
 *
 * Зачем это нужно вообще. Будильник живёт в Android, а не в приложении, и
 * переживает не всё: обновление приложения из Play снимает все запланированные
 * alarm'ы, и напоминание, включённое месяц назад, после первого же обновления
 * молча перестаёт приходить. Снаружи это выглядит ровно как «переключатель
 * стоит на «Вкл», а уведомлений нет» — то есть как сломанная настройка.
 *
 * Второе, ради чего: текст. Он собирается в момент планирования и замерзает,
 * поэтому напоминание, поставленное на сотой странице, полгода зовёт назад на
 * сотую страницу. Перестановка при каждом запуске обновляет и номер.
 *
 * Разрешение здесь именно ПРОВЕРЯЕТСЯ, а не запрашивается: системный диалог
 * показывается один раз за всю жизнь установки, и потратить его на старте
 * приложения — значит отобрать его у того места, где человек сам нажал «Вкл».
 *
 * @param {{title?: string, body?: string}} [payload]
 * @returns {Promise<string>} один из NOTIFY. Исключений не бросает.
 */
export async function refreshNotifications(payload = {}) {
  ...
  try {
    const {display} = await plugin.checkPermissions();
    // Разрешение можно отозвать в настройках телефона, и приложение об этом
    // не узнаёт никак, кроме вот этой проверки. Вызывающий гасит переключатель.
    if (display !== 'granted') return NOTIFY.denied;
    await schedule(plugin, payload);
    return NOTIFY.on;
  } catch (err) {
    console.warn('[native] не удалось переставить напоминание:', err);
    return NOTIFY.failed;
  }
}
