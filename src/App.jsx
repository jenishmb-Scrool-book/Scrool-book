...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.resolve(initNative({onBack}))
      ...
      .catch(err => {
        // Молчать здесь нельзя. Ровно так и потерялась аппаратная «назад»:
        // initNative падал на телефоне первой же строкой, приложение об этом
        // не говорило ни слова, а снаружи это выглядело как «кнопка не
        // работает» — то есть как что угодно, кроме исключения при старте.
        console.warn('[app] нативный слой не поднялся:', err);
      });
    return () => {
      dead = true;
