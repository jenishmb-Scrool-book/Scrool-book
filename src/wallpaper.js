...
// Готовые обои — из тех же снимков, которыми набиты ленты.
//
// Ни одного нового файла в сборке: все двадцать уже лежат в `public/pics/tall`
// и весят столько же, сколько весили до этого списка. Вертикальная лента
// взята потому, что там кадр 9:16 — ровно тот, что нужен на весь экран.
//
// Номера выбраны глазами из всех 195, а не по правилу: обои — это фон под
// часами, виджетом и двумя рядами значков, поэтому годятся спокойные кадры без
// сюжета в середине и без читаемых надписей. Внутри списка нарочно разные
// настроения: тёмные и светлые, тёплые и холодные, пустые и плотные.
//
// Счёт снимков в `pic()` начинается с нуля, а имена файлов — с единицы,
// поэтому здесь номер файла минус один.
const PICKED = [3, 18, 19, 24, 27, 38, 39, 43, 52, 59,
                61, 69, 79, 90, 105, 108, 118, 133, 134, 139];

/** Двадцать готовых обоев, путями для `url()`. */
export const WALLS = PICKED.map(n => pic('tall', n - 1));
